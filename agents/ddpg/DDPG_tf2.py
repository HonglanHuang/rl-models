...
class DDPG:
    def __init__(self,
        ...
        self.Actor = ActorNet(self.sess, self.state_dim, self.action_dim, self.action_range, 400, 300, self.lr_a, self.tau, self.batch_size)
        self.Critic = CriticNet(self.sess, self.state_dim, self.action_dim, 400, 300, self.lr_c, self.tau)
        self.Target_Actor = ActorNet(self.sess, self.state_dim, self.action_dim, self.action_range, 400, 300, self.lr_a, self.tau, self.batch_size)
        self.Target_Critic = CriticNet(self.sess, self.state_dim, self.action_dim, 400, 300, self.lr_c, self.tau)
...
class ActorNet:
    ...
    def train(self, states, action_grads):
        ...
        self.train(states, action_grads)

    def train_op(self):

        return K.function([self.model.input, self.action_grads], [self.optimizer.apply_gradients(self.grads)])
